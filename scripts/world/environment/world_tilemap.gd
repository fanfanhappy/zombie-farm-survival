...
func _index_farm_plots() -> void:
	...
	for node in get_tree().get_nodes_in_group("farm_plots"):
		...
		farm_plots_by_cell[world_to_farm_cell(plot.global_position)] = plot
...
func _refresh_farming_terrain(force := false) -> void:
	...
	for node in get_tree().get_nodes_in_group("farm_plots"):
		...
		if plot.state != FarmPlot.PlotState.EMPTY:
			next_cells.append(WorldGrid.layer_world_to_cell(farming_layer, plot.global_position))
	next_cells.sort()
...
func register_farm_plot(plot: FarmPlot) -> void:
	var cell := world_to_farm_cell(plot.global_position)
...
func unregister_farm_plot(plot: FarmPlot) -> void:
	_on_farm_plot_exiting(world_to_farm_cell(plot.global_position), plot)
...
func is_world_position_tillable(world_position: Vector2) -> bool:
	return is_cell_tillable(world_to_farm_cell(world_position))


func world_to_farm_cell(world_position: Vector2) -> Vector2i:
	return WorldGrid.layer_world_to_cell(ground_layer, world_position)


func farm_cell_to_world(cell: Vector2i) -> Vector2:
	return WorldGrid.layer_cell_to_world(ground_layer, cell)


func snap_to_farm_cell(world_position: Vector2) -> Vector2:
	return WorldGrid.snap_to_layer(ground_layer, world_position)


func get_farm_cell_size() -> float:
	return WorldGrid.get_layer_cell_size(ground_layer)
...
func get_till_block_reason(cell: Vector2i) -> String:
	var world_position := farm_cell_to_world(cell)
	var half_cell := get_farm_cell_size() * 0.5
	if world_settings == null or not world_settings.world_bounds.grow(-half_cell).has_point(world_position):
		return "超出可操作的地图范围"
...
func _has_blocking_world_object(world_position: Vector2) -> bool:
	var farm_cell_size := get_farm_cell_size()
	...
	shape.size = Vector2.ONE * maxf(farm_cell_size - 4.0, 1.0)
...
func _update_grid_cursor() -> void:
	...
	if is_instance_valid(hovered_plot):
		...
		return
	var snapped_position := snap_to_farm_cell(mouse_position)
	var cell := world_to_farm_cell(snapped_position)
...
func _find_hovered_farm_plot(mouse_position: Vector2) -> FarmPlot:
	var cell := world_to_farm_cell(mouse_position)
	var plot := farm_plots_by_cell.get(cell) as FarmPlot
	if plot == null or mouse_position.distance_to(plot.global_position) > get_farm_cell_size() * 0.5: return null
	return plot
